...
func main() {
	...
	firestoreClient, _ := libraries.FirebaseApp().Firestore(ctx)
	// prepareQuery(ctx, firestoreClient)
	multipleDocs(ctx, firestoreClient)
}

func multipleDocs(ctx context.Context, client *firestore.Client) error {
	// [START fs_get_multiple_docs]
	iter := client.Collection("cities").Where("Capital", "==", true).Documents(ctx)
	// getCitie, _ := client.Collection("cities").Doc("LA").Get(ctx)
